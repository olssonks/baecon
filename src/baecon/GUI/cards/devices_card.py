...
def main(
    ...
    with ui.column().classes("w-full h-full place-content-around"):
        ...
        with ui.column().classes("w-full"):
            ...
            async def add_fix(e):
                await add_device_button(e, meas_settings)

            ui.button(add_label, on_click=add_fix).classes("w-full")
            ui.button(
                remove_label, on_click=lambda e: remove_device_button(e, meas_settings)
            ).classes("w-full")
    return
...
async def add_device_button(click_event, meas_settings: bc.Measurement_Settings):
    ...
    await add_device_dialog(meas_settings)
    return


async def add_device_dialog(meas_settings: bc.Measurement_Settings):
...
async def choose_device(
    ...
    """Create tables of devices available in the `Devices` directory.
    ...
    if name_input.value == "":
        ui.notify(
            "Please Choose Working Name", position="center", type="negative", timeout=1000
        )
    else:
        row = await grid.get_selected_row()
        device_type_holder.update(row["name"])
        add_device_configs(device_name_holder, device_type_holder, meas_settings)

        if device_gui_enabled.value:
            open_device_gui(device_name_holder, meas_settings)

        dialog.close()
    return


def add_device_configs(
    device_name_holder: gui_utils.holder,
    device_type_holder: gui_utils.holder,
    meas_settings: bc.Measurement_Settings,
):
    ...
    Args:
        ...
        meas_settings (bc.Measurement_Settings): Object defining the entire
            measurement.
    """
    default_config = get_device_default(device_name_holder, device_type_holder)

    if devices_catagory_holder.value == "Scan Devices":
        meas_settings.scan_devices.update({device_name_holder.value: default_config})
        bc.add_device(default_config, meas_settings.scan_devices)
    else:
        meas_settings.acquisition_devices.update({device_name_holder.value: default_config})
        bc.add_device(default_config, meas_settings.acquisition_devices)
    ...
    update_devices_card(meas_settings)
    return


def open_device_gui(device_name_holder, meas_settings):
    if devices_catagory_holder.value == "Scan Devices":
...
def get_device_default(
    device_name_holder: gui_utils.holder, device_type_holder: gui_utils.holder
) -> dict:
    ...
    if not found_default:
        print(
            f"No default configuration file found for {device_type_holder.value} in {device_path}.\
            Using defaults in defined in module."
        )
        default_config = {}
...
def remove_device_button(click_event, meas_settings: bc.Measurement_Settings):
...
async def save_button(meas_settings: bc.Measurement_Settings):
    ## get the device name from the label given to the expansion element in show_devices
    device_name = (
        devices_card_holder.value[devices_catagory_holder.value]
        .default_slot.children[0]
        .default_slot.children[0]
        ._props["label"]
    )
...
def device_tabs(
    device_name: str, device_configuration: dict, meas_settings: bc.Measurement_Settings
):
...
def list_parameters(
    ...
    with ui.column().classes("w-full"):
        ui.button("Launch Device GUI", on_click=lambda: open_device_gui(device_name_holder, meas_settings)).classes("w-full")
        with ui.row().classes("w-full no-wrap"):
            ui.button("Save Device", on_click=partial(save_button, meas_settings)).classes(
                "w-full"
            )
