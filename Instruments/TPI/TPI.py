...
class TPI():
    ...
    def __init__(self):
        ...
        self.sp_default = {'freq_func' : 0, ## no scan
    ...
    def SetMWAmpl(self, ampl_setting):
        ...
        ampl_hex = '{:0x}'.format(ampl_setting & 0xFF)
        ...
        ampl_ret_u = int(ret_msg[-4:-2], 16) ## yield unsigned integer
    ...
    def SetMWFreq(self, freq_setting):
        ...
        freq_hex = '{:08x}'.format(freq_setting & 0xFF)
    ...
    def SetMWSet(self, stat_setting):
        ## ampl_setting will be in dBm from -90 to +10
        ## convert int to unsigned int, then convert to single byte hex value
        stat_hex = '{:0x}'.format(stat_setting & 0xFF)
    ...
    def SetScanParams(self):
        ...
        scan_msg += self.LSB_order('{:04x}'.format(int(self.sp['ampl_high']
                                                        & 0xFF)))
        scan_msg += self.LSB_order('{:04x}'.format(int(self.sp['ampl_low']
                                                        & 0xFF)))
...
### Utility Functions
    ...
    def hex_message(self, cmd, setting=[]):
        ...
        instruction_length = '{:04x}'.format(len(inst_array))
    ...
    def perform_checksum(self, byte_array):
        ...
        ## four character hex value denoting number of bits in instruction
        array_length = '{:04x}'.format(len(byte_array))
    ...
    def uint8_to_int8(uinteger):
        binary_str = '{:08b}'.format(uinteger)
