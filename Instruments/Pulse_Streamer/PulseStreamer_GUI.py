...
class Pulse_Sequence: ## Maybe different name to not be confused with PulseStreamer sequencies
    ...
    def sort_pulses(self, channel_index):
...
def edit_pulse_card():
    ...
    with ui.column().classes('w-full h-full'):
        ...
        with ui.row().classes('w-full h-full no-wrap'):
            ...
            ui.input('Type', value="None").classes('w-1/3')\
                .bind_value(ps_gui_holder, 'pulse_type').classes('text-uppercase')
...
def update_pulse_information():
    if not ps_sequence.pulses == []:
        pulse_index     = ps_gui_holder.pulse_number
        start = ps_sequence.pulses[ps_gui_holder.channel_choice][pulse_index].start_time
        duration = ps_sequence.pulses[ps_gui_holder.channel_choice][pulse_index].duration
...
def add_pulse(ps:Pulse_Sequence, 
    ...
    shift_dict[shift_type](ps, channel_choice, start, duration, type)
    plot_sequence()
    return
...
def pulse_insert(ps:Pulse_Sequence, channel_choice:int, 
    ...
    new_pulse     = Pulse(type, float(duration), float(start), 
    ...
    for ch_idx, chan in enumerate(ps.pulses):
        for p_idx, pulse in enumerate(chan):
            if pulse.start_time > start:
                ps.pulses[ch_idx][p_idx] = Pulse(type, duration, start, 
                                                 start + duration)#(pulse[0]+duration, pulse[1])
    ps.total_duration = sequence_duration(ps.pulses)
    return
...
def shift_channel(ps:Pulse_Sequence, channel_index:int, 
    ...
    for p_idx, pulse in enumerate(ps.pulses[channel_index]):
        if pulse.start_time > start:
            ps.pulses[channel_index][p_idx] = Pulse(type, duration, start, 
                                                    start + duration)
    ps.total_duration = sequence_duration(ps.pulses)
    return
...
shift_dict = {'no_shift': no_shift, 'shift_all': shift_all, 
...
def make_plot_data(ps_sequence:Pulse_Sequence):
    seq_duration = ps_sequence.total_duration
    all_pulses = ps_sequence.pulses
...
def plot_pulse_types(ps_sequence:Pulse_Sequence,
    ...
    for ch_idx, types in enumerate(ps_sequence.types):
        for p_idx, type in enumerate(types):
            time_idx = int(ps_sequence.pulses[ch_idx][p_idx].start_time)*1000
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    ui.button('yolo', on_click=print_seq)
